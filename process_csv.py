...
def process_row(row, column_indexes, data):
    device_type = row[column_indexes["device type"]]
    for day in list(data.keys()):
        initialise_day_data(day, device_type, data)
        add_registration_to_data(day, device_type, data, row)


def add_registration_to_data(day, device_type, data, row):
    reg_count = 0
...
def initialise_day_data(day, device_type, data):
...
def get_tcp_stores(closed_stores_filename="Closed_DG_Stores_27_May.xlsx"):
    wb = openpyxl.load_workbook(os.path.join("docs", closed_stores_filename))
    closed_sheet = wb["tcp shops"]
    return [row[0].value for row in closed_sheet.rows if row[0].value]
